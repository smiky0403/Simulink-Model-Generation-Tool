function valid = has_inport(dir, varargin)
%--------------------------------------------------------------------------
...
% Example Run: valid = has_inport(gcs, {'a', 'b', 'c', 'd', 'd0', 'd1', 'd2', 'u', 'v', 'e'})
...
for i = 1: length(name_blks)
   ...
   if strcmp(type_blk, 'SubSystem')   
        ...
        path_sub_blks = strcat(path_blks{i}, '/', sub_blks);
        type_sub_blks = get_param(path_sub_blks, 'BlockType');
        
        if any( strcmp(type_sub_blks, 'Inport')) 
             valid(i) = true;
        else
            continue
        end
